...
Since Transformers.js can run in the browser or in Node.js, you can choose whether you want to perform inference [client-side](#client-side-inference) or [server-side](#server-side-inference).
...
To make our application fully static, we'll need to make a few changes to the default project skeleton.
You need to install the [adapter-static](https://github.com/sveltejs/kit/tree/master/packages/adapter-static) to prerender your application as a collection of static files.
...
Next, we'll create a new [Web Worker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers) script where we'll place all ML-related code.
This is to ensure that the main thread is not blocked while the model is loading and performing inference.
...
We'll define the main interface and logic for our application on the single page component `./src/routes/+page.svelte` that maps to `/`.
Learn more how SvelteKit handles routing [here](https://kit.svelte.dev/docs/routing).

Let's started by loading the Web Worker as soon as the page is mounted. In Svelte, we can use the `onMount` function to run code when the component is mounted.
The worker code is referenced via the alias `$lib/worker.js`. Since SvelteKit is build with Vite.js, we can use the special syntax `?worker` to indicate that this file should be loaded as a Web Worker.
Read more here [Vite.js Web Workers](https://vitejs.dev/guide/features.html#web-workers).
...
If you have a simple web server installed, you can serve the static files, i.e. [live-server](https://www.npmjs.com/package/live-server):

```bash
live-server ./build
```

Go to [http://localhost:8080/](http://localhost:8080/) to see your application in action!
...
For server-side inference, we'll rely on [SvelteKit API routes](https://kit.svelte.dev/docs/routing#server) to handle requests.
...
We'll define the main interface and logic for our application on the single page component `./src/routes/+page.svelte` that maps to `/`.
The main difference with the client-side inference is that we'll be using the `fetch` API to make requests to our API route handler, rather than using a Web Worker.
...
Deploying as a Docker container on [Hugging Face Spaces](https://huggingface.co/docs/hub/spaces). If you haven't already, you can create a free Hugging Face account [here](https://huggingface.co/join).

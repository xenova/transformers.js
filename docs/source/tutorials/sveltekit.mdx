...
Let's started by loading the Web Worker as soon as the page is mounted. On Svelte, we can use the `onMount` function to run code when the component is mounted.
The worker code is referenced via the alias `$lib/worker.js`, since SvelteKit is build with Vite.js, we can use its special syntax `?worker` to indicate that this file should be loaded as a Web Worker.

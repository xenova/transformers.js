...
Transformers.js will attach an Authorization header to requests made to the Hugging Face Hub when `process.env.HF_ACCESS_TOKEN` is set.

One way to do this is to call the running process with the environment variable set. For example,let's say you have a file called `testing.js` with the following code:

```js
import { AutoTokenizer } from '@xenova/transformers';

// Load tokenizer for a gated model.
...
You can use the following command to set the environment variable and run the file:
